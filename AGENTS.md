...
- `game.py`: main game loop, constants, entities, UI, and persistence hooks.
- `assets/sprites.py`: procedural textures/effects (PIL + Arcade). Imported by `game.py`.
- `db_view.py`: read‑only helper to inspect SQLite scores (`warrior_platform.db`).
- `assets/__init__.py`: package marker. No external asset files required.
- Generated at runtime: `warrior_platform.db`, `scores.txt` (both in repo root).

## Build, Test, and Development Commands
- Create env: `python -m venv .venv && source .venv/bin/activate` (Windows: `.venv\Scripts\activate`).
- Install deps: `pip install arcade pillow`.
- Run the game: `python game.py` (starts fullscreen; creates/updates DB/scores).
- Inspect DB: `python db_view.py` (lists players and top scores).
- Package (Windows): use auto‑py‑to‑exe as described in `README.md`.

## Coding Style & Naming Conventions
- Python, PEP 8, 4‑space indents. Keep identifiers in English; comments/docs may be PT‑BR to match README.
- Functions/variables: `snake_case`; constants: `UPPER_SNAKE_CASE`; classes (if added): `PascalCase`.
- Prefer small, cohesive functions. Keep frame logic in `on_update` lightweight.
- Optional tools: Black (`black .`) and Ruff (`ruff .`). Keep diffs minimal.

## Testing Guidelines
- Framework: `pytest` (suggested). Name files `tests/test_*.py`.
- Focus on pure‑Python units: procedural sprite helpers (`assets.sprites`), DB helpers (`db_view.py`).
- Skip/mark integration that requires a window/GL context (Arcade render). Example: `pytest -q`.
- Target ≥80% coverage for new code touching logic/state.

## Commit & Pull Request Guidelines
- Language: keep commit messages in PT‑BR, imperative and concise (e.g., “Ajusta IA do morcego”).
- Scope: one change per commit when possible; reference issues (`#123`).
- PRs must include: summary do que mudou, motivo, passos de teste locais, e captura/GIF se afetar gameplay/UI.
- Avoid committing generated files: `warrior_platform.db`, `scores.txt`, builds in `dist/`.

## Security & Configuration Tips
- SQLite writes to repo root; ensure the game runs from a writable folder. Do not store secrets.
- For packaging, include Arcade GL backends (hidden‑imports) and consider `ARCADE_GL_PROVIDER=pyglet` per README.
- Keep performance in mind: cache text/sprites as in `game.py` and avoid per‑frame allocations.

## Referência Completa (README.md)
O conteúdo abaixo replica o README.md do projeto para referência integral.
